...
public class ClientPane extends BorderPane {
	...
	public ClientPane()
	{
		...
		OrderPane.add(new Label("Kg:"), 0, 5);
		OrderPane.add(new Label("Address:"), 0, 9);
		OrderPane.add(new Label("Contact:"), 0, 10);
		...
		OrderPane.add(txtKg, 1, 5);
		OrderPane.add(btnAdd, 1, 6);
		OrderPane.add(txtAddress, 1, 9);
		OrderPane.add(txtContact, 1, 10);
		...
		vorder.getChildren().addAll(new Label("Enter Text Here:"),ta,Confirmbox);
		...
		btntrans.setOnAction(e-> BlockTrans());
	...
	public void AddOrder()
	{
		...
		ll.addLast('\s'+"Customer Address:" +'\s' + txtAddress.getText());
		ll.addLast('\s' +"Customer contact:" +'\s' + txtContact.getText());
	...
	//creates a new Transaction
			...
			{
				 trans = new Transaction<String>("Client","Admin",ta.getText());
				
				
				//setting the data in the transaction
				trans.setData(ta.getText()); 
				trans.setSender("Client");
				trans.setReceiver("Admin");
				//ll.add(trans);
				list.AddFirst(trans);
			...
			{
				//sends commands and list of transactions
				pw.println("SEND"+'\s'+"Client"+ list.toString());//+trans.toString());//+txtBlock.getText());
				pw.flush();
				AddBlock();
		...
		//Getting my blocks
		public void BlockTrans()
		{
			
			
			for(Transaction<String> t: block.getTransactions())
			{
			txtBlock.setText(t.toString());
			}
		...
		//Edits the data in the Blocks and creates block
		public Block<String> AddBlock()
		{
			//creating a block for client
			block = new Block<String>("Client",list);
