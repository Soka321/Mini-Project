...
public class ManAdmin extends BorderPane
{
	PrintWriter pw= null;
	Socket socket = null; 
	...
	public ManAdmin()
	{
		...
		vorder.getChildren().addAll(ta,Confirmbox);
		...
		blockBox.getChildren().addAll(btnBlock,btnRemove,btntrans,new Text("Enter block number:"),txtIndex);
	...
	public void AddOrder()
	{
		...
		ll.addFirst("Product ID:"+'\s' +txtProductId.getText());
		ll.addFirst('\s' +"Product name:"+'\s' +txtProductname.getText());
		ll.addFirst('\s' +"Type:"+'\s' +txtType.getText());
		ll.addFirst('\s' +"Quantity:"+'\s' +txtQuantity.getText());
		ll.addFirst('\s' +"Price:"+'\s' +txtPrice.getText());
		ll.addFirst('\s' +"Kgs:" +'\s' + txtKg.getText());
	...
	//creates a new Transaction
			...
			{
				 trans = new Transaction<String>("Admin","Manufacturer",ta.getText());
				
				
				//setting the data in the transaction
				trans.setData(ta.getText()); 
				trans.setSender("Admin");
				trans.setReceiver("Manufacturer");
				//ll.add(trans);
				list.AddFirst(trans);
			...
			public void sendOrder()
			{
				...
				pw.println("SEND"+'\s'+"Admin"+'\s' +list.toString());//+trans.toString());//+txtBlock.getText());
				pw.flush();
		...
		{
			pw.println("GET"+'\s'+"Admin");
			pw.flush();
			inbox.appendText("Getting transactions fro block");
		...
		//Edits the data in the Blocks and creates block
				public Block<String> AddBlock()
				{
					...
					block = new Block<String>("Admin",list);
